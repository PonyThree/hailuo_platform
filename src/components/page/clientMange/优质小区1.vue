<template>
    <div style='margin-top:30px;' class='box' v-loading="loadCoperationList">
                ...
                <div v-for='(item,index) in coperationList' :key="index" class="list">
                    <div class="listItem">
                            <div class="upload_warp" >
                                <div class="upload_warp_left img-upload" @click="homeImg_file(1,item.id)">
                                    <img :src="item.joinHandsImage" v-if="item.joinHandsImage" class="avatar mt0">
                                </div>
                            </div>
                            <input @change="homeImg_fileChange($event,item.id)" type="file" id="homeImg_upload_file"  style="display:none;" value="1"/>
                        <p class="pname">{{item.name}}</p>
                        <p class="address">{{item.provinceName}}{{item.cityName}}{{item.countyName}}</p>
                        <p v-show="1==2">{{item.id}}</p>
                    </div>
                    <p @click='jumpPD1(index,item.id,item.frontShow,item.publishStatus)' class="showStyle" :style="{backgroundColor:item.publishStatus==0?'#ccc':'rgb(64,159,255)'}">{{item.frontShow==1?'不展示':'展示'}}</p>
                </div> 
            </div>
        </div>
        <!-- 已发布项目 -->
        <div class="publishPro">
            ...
            <div class='content' >
                <div v-for='(item,index) in publishProject' :key="index" class="list">
                    <div class="listItem">
                        <!-- <div class="upload_warp" v-if="item.joinHandsImage">
                        ...
                        <div class="pic-box">
                            <div class="upload_warp" >
                                <div class="upload_warp_left img-upload" @click="homeImg_file(2)">
                                    <img :src="item.joinHandsImage" v-if="item.joinHandsImage" class="avatar mt0">
                                </div>
                            </div>
                            <input @change="homeImg_fileChange1($event,item.id)" type="file" id="homeImg_upload_file1"  style="display:none;"/>
                        </div>
                    ...
                    <p @click='jumpPD(index,item.id,item.frontShow,item.publishStatus)' class="showStyle" :style="{backgroundColor:item.publishStatus==0?'#ccc':'rgb(64,159,255)'}">{{item.frontShow==1?'不展示':'展示'}}</p>
                </div> 
            </div>
            <!--分页器-->
        ...
        <!-- 新增弹出框 -->
        <!-- <el-dialog title="图片上传:" :visible.sync="addVisible" width="20%" @close="insertClose">
...
export default {
    ...
    methods:{
        ...
        homeImg_file(num,id){
            ...
            if(num==2){
                document.getElementById('homeImg_upload_file1').click()
            }   
        },
        homeImg_fileChange(e,id){
            ...
            .then(res => {
                if(res.data.code==0){
                    // console.log(res.data.data);
                    // this.formList.imgUrl=res.data.data;
                    console.log(this.coperationList)
                    this.coperationList.forEach(item=>{
                        if(item.id==id){
                            item.joinHandsImage=res.data.data
                        }
                    })
                    // this.coperationList[id].joinHandsImage=res.data.data
                    this.$message({
                        type: 'info',
                        message: "图片上传成功！"
                    });	
        ...
        homeImg_fileChange1(e,id){
            console.log(this.publishProject)
            console.log(id)
            var param = new FormData(); // FormData 对象
            param.append("file", e.target.files[0]); // 文件对象
            this.$axios({
            ...
            .then(res => {
                if(res.data.code==0){
                    // console.log(res.data.data);
                    // this.formList.imgUrl=res.data.data;
                    // this.publishProject[id].joinHandsImage=res.data.data

                    this.publishProject.forEach(item=>{
                        if(item.id==id){
                            item.joinHandsImage=res.data.data
                        }
                    })
