    ...
    <div class="indexPage">
      ...
      <div class="title">
        ...
        <div class="head">
          ...
          <el-button class="logout" v-if="show" @click="logot"
            >退出登录</el-button
          >
        </div>
      </div>
      ...
      <div class="content">
        <div class="list" v-for="(item, index) in projectList" :key="index">
          ...
          <div class="btn">
            <el-button type="primary" @click="jumpPage(index + 1)">{{
              item.jumpTxt
            }}</el-button>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    jumpPage(i) {
      // alert(i);
      if (i == 1) {
        this.$router.push("/项目管理");
      }
      if (i == 2) {
        this.$router.push("/设置");
      }
      if (i == 3) {
        this.$router.push("/客户端管理");
      }
      if (i == 4) {
        this.$router.push("/城市管理");
      }
      if (i == 5) {
        this.$router.push("/项目财务管理");
      }
    }
  }
};
