<template>
  <div id="userWallet">
    ...
    <el-table :data="tableData" border style="width: 94%">
      ...
      <el-table-column prop="userName" label="客户昵称" align="center">
      </el-table-column>
      <el-table-column prop="mobile" label="联系电话" align="center">
      </el-table-column>
      <el-table-column prop="withdraw" label="提现详情" align="center">
        <template slot-scope="scope">
          <el-button
            @click.native.prevent="withdraw(tableData[scope.$index].userId)"
            type="text"
            size="small"
            style="color:#409EFF;"
          >
      ...
      <el-table-column prop="operating" label="操作状态" align="center">
        <template slot-scope="scope">
          <el-button
            @click.native.prevent="details(tableData[scope.$index].userId)"
            type="text"
            size="small"
            style="color:#409EFF;"
          >
    ...
    <div class="totalNum">总计：￥{{ moneyCount }}</div>
    <!--分页器-->
    <el-pagination
      background
...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      this.$axios
        .get(request.testUrl + "/user/auth2/userMoney/listUserMoneyInfo", {
          params: obj
        })
        .then(res => {
    ...
    renderMoney() {
      this.$axios
        .get(request.testUrl + "/user/auth2/user/moneyStatistical")
        .then(res => {
