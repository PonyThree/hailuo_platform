<template>
  <div id="moneyWithdraw">
    ...
    <el-table :data="tableData" border style="width: 94%">
      ...
      <el-table-column prop="applyForTime" label="提现时间" align="center">
        <!-- <template slot-scope="scope">
            <div>
                <span>{{time}}</span>
            </div>
      ...
      <el-table-column prop="cashMoney" label="提现金额" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column prop="orderNo" label="交易单号" align="center">
      </el-table-column>
      <el-table-column prop="arriveAddress" label="到账地址" align="center">
      </el-table-column>
      <el-table-column prop="withdrawalDetails" label="提现详情" align="center">
        <template slot-scope="scope">
          <el-button
            @click.native.prevent="details(tableData[scope.$index].recordId)"
            type="text"
            size="small"
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$axios
        .get(
          request.testUrl + "/user/auth2/userCashRecording/userCashRecordList",
          {
            params: obj
          }
        )
          ...
          if (res.data.data.records.length >= 1) {
            ...
            for (var i = 0; i < res.data.data.records.length; i++) {
              if (res.data.data.records[i].applyForTime != "") {
                this.tableData[i].applyForTime = this.transformDate(
                  res.data.data.records[i].applyForTime
                );
              } else {
