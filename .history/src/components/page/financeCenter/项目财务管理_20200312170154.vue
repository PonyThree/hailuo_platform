<template>
  <div id="financeManagement">
    ...
    <!-- 表格 -->
    <el-table :data="tableData" border style="width: 94%" v-loading="loading">
      <el-table-column
        prop="projectName"
        label="项目名称"
        width="100"
      ...
      <el-table-column prop="lwMoney" label="落位收入（元）" align="center">
      </el-table-column>
      <el-table-column prop="rgMoney" label="认购收入(元）" align="center">
      </el-table-column>
      <el-table-column prop="tkMoney" label="所有退款（元）" align="center">
      </el-table-column>
      <el-table-column label="平台总收入（元）" align="center" prop="ptMoney">
      </el-table-column>
      <el-table-column prop="ptMoney" label="项目总收入（元）" align="center">
      </el-table-column>
      <el-table-column
        prop="xmProposed"
        label="项目已提现（元）"
      ...
      <el-table-column prop="withdrawalsRecord" label="提现记录" align="center">
        <template slot-scope="scope">
          <el-button
            @click.native.prevent="
              viewRecord(tableData[scope.$index].projectId)
            "
            type="text"
            size="small"
            style="color:#409EFF;"
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            size="small"
            style="color:#ccc;"
            v-if="tableData[scope.$index].stype == 0"
          >
            暂无
          </el-button>
          <el-button
            ...
            v-else
            @click.native.prevent="
              viewWithdrawal(tableData[scope.$index].appId)
            "
          >
            查看
          </el-button>
...
export default {
  ...
  methods: {
    ...
    renderDate() {
      ...
      this.$axios
        .post(request.testUrl + "/order/auth2/orderForm/getPlaSumData", params)
        .then(res => {
          if (res.data.code == 0) {
            this.loading = true;
            var lock = setTimeout(() => {
    ...
    onSubmit() {
      if (!this.form.input5) {
        ...
        this.$axios
          .post(
            request.testUrl + "/order/auth2/orderForm/getPlaSumData",
            params
          )
          .then(res => {
