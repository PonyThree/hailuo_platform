          ...
          <el-row class="infoDetails">
            ...
            <el-col :span="8">
              <div class="infoDetailsdes">
                提现金额：<span class="fontRed">￥{{ cashMoney }}</span>
              </div>
            </el-col>
          </el-row>
          <el-row class="infoDetails">
...
export default {
  ...
  methods: {
    renderData(id) {
      this.$axios
        .get(
          request.testUrl + "/user/auth2/userCashRecording/userCashRecordInfo",
          {
            params: {
              id: id
            }
    ...
    desc() {
      return (
        "提现申请时间" +
        "-------" +
        this.transformDate(this.applyForTime) +
        "-------" +
        "提现状态" +
        "-------" +
        this.statusName +
        ...
        "提现到账时间" +
        "------" +
        this.transformDate(this.arriveTime)
      );
    },
    errDesc() {
      return (
