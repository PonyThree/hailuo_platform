  ...
  <div>
    ...
    <el-table :data="tableData" border style="width: 100%">
      ...
      <el-table-column prop="caozuo" label="操作" align="center">
        <template slot-scope="scope">
          <el-button icon="el-icon-edit" @click="handleEdit(scope.$index)"
            >编辑</el-button
          >
          <el-button
            icon="el-icon-delete"
        ...
        <el-form-item label="图片" style="width:92%;">
          ...
          >
            ...
            <img
              ...
              alt=""
              style="width:10%;height:25%;position: absolute; right:0px;top:5px;"
              @click="show"
            />
          </div>
    ...
    <el-dialog title="新增" :visible.sync="addVisible" width="30%">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="addEdit('formList')" class="flag"
          >保存</el-button
        >
  ...
  methods: {
    ...
    save() {
      this.$axios
        .get(
          request.testUrl +
            "/activityBanner/pageSelect?page=" +
            this.currentPage +
            "&pageSize=" +
            this.pagesize
        )
    ...
    addEdit(formList) {
      ...
      //              console.log(params)
      this.$axios
        .post(request.testUrl + "/activityBanner/insert", params)
        .then(res => {
    ...
    handleEdit(index) {
      ...
      //console.log(id)
      this.$axios
        .post(request.testUrl + "/activityBanner/getOne", params)
        .then(res => {
          if (res.data.code == 0) {
    ...
    savePic() {
      ...
      this.$axios
        .post(request.testUrl + "/activityBanner/update", params)
        .then(res => {
          if (res.data.code == 0) {
            //console.log(res)
    ...
    deleteMember(index) {
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          params.append("id", id);
          this.$axios
            .post(request.testUrl + "/activityBanner/delete ", params)
            .then(res => {
              //console.log(res)
              this.save();
            });
