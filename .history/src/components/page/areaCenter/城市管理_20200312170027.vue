    ...
    <div style="margin:20px auto 0;width:80%;overflow:hidden;">
      ...
      >
        <el-table-column
          ...
          prop="checked"
        ></el-table-column>
        <el-table-column label="区域" prop="provinceName"> </el-table-column>
        <el-table-column label="城市" prop="cityName"> </el-table-column>
        <el-table-column label="地区" prop="countyName"> </el-table-column>
        <el-table-column label="项目名称" prop="projectName"> </el-table-column>
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button
              type="text"
              style="color:rgb(25,158,216);"
              @click="find(scope.$index, areaData[scope.$index].id)"
              >查看</el-button
            >
          </template>
        </el-table-column>
  ...
  methods: {
    ...
    renderAreaList(n1, n2) {
      this.$axios
        .get(
          request.testUrl +
            `/project/auth2/project/projectList?page=${this.page}&pageSize=${this.pageSize}`
        )
        .then(res => {
    ...
    find(i, id) {
      ...
      //获取初始化的数据
      this.$axios
        .get(request.testUrl + "/project/auth2/project/selectOne", {
          params: {
            projectId: id
          }
        })
    ...
    getProvinceList() {
      this.$axios
        .get(request.testUrl + "/project/sysRegion/select", {
          params: {
            parentId: 0
          }
        })
