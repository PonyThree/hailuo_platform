    ...
    >
      ...
      <el-table-column prop="imgUrl" label="图片" align="center">
        <template slot-scope="scope">
          <img
            ...
            style="width: 286px;height: 100px;"
          />
      ...
      <el-table-column prop="caozuo" label="操作" align="center">
        <template slot-scope="scope">
          <el-button
            icon="el-icon-edit"
            @click="handleEdit(scope.$index, bannerTable[scope.$index].id)"
            >编辑</el-button
          >
          <!-- v-if="scope.$index!=0" -->
          <el-button
            icon="el-icon-delete"
            class="red"
            @click="deleteMember(scope.$index, bannerTable[scope.$index].id)"
            v-if="scope.$index != 0"
            >删除</el-button
          >
        </template>
    ...
    <el-dialog title="编辑" :visible.sync="editVisible" width="26%">
      ...
      >
        <el-form-item label="图片" style="width:86%;">
          <div
            style="width: 80%;height: 150px;display: block;float: left;position: relative;"
            id="aa"
          >
            <img
              :src="form.imgUrl"
            ...
            <img
              ...
              alt=""
              style="width:10%;height:25%;position: absolute; right:0px;top:-3px;"
              @click="show"
            />
          </div>
        ...
        <el-form-item label="图片上传:" style="width:92%;" prop="image">
          ...
          >
            ...
            <img
              src="../../../assets/img/2.png"
              alt=""
              style="width:10%;height:25%;position: absolute; right:-7px;top:-4px;"
              @click="show"
            />
          </div>
...
export default {
  ...
  methods: {
    showBannerList() {
      this.loading = true;
      this.$axios
        .get(request.testUrl + "/platform/auth2/platformBanner/pageSelect", {
          params: {
            page: this.current,
            pageSize: 10
          }
    ...
    addEdit(formList) {
      ...
      this.$axios
        .post(
          request.testUrl + "/platform/auth2/platformBanner/doInsert",
          params
        )
        .then(res => {
          if (res.data.code == 0) {
    ...
    handleEdit(index, id) {
      ...
      this.$axios
        .get(request.testUrl + "/platform/auth2/platformBanner/getOne", {
          params: {
            id: id
          }
        })
    ...
    deleteMember(index, id) {
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          this.$axios
            .post(
              request.testUrl + "/platform/auth2/platformBanner/doDelete ",
              params
            )
            .then(res => {
