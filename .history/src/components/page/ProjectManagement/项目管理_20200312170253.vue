  ...
  <div class="manage" v-loading="loading">
    ...
    <div class="search">
      ...
      <el-form
        :inline="true"
        :model="form"
        class="demo-inline searchIdea searchTxt"
      >
      ...
      <div class="btns">
        ...
        <el-button @click="export1" style="color:#00f;" type="primary"
          >导出</el-button
        >
      ...
      <el-table
        ...
        style="width: 100%;display:block;margin-left: 0 auto;"
        align="center"
        label-width="180px"
      ...
      >
          ...
          <template slot-scope="scope">
            <div>
              <span style="margin-right:16px">{{
                tableDatass[scope.$index].projectName
              }}</span>
              <div
                class="topIcon"
                v-if="tableDatass[scope.$index].showStatus == 1"
              >
                <img
                  src="../../../assets/img/publish.png"
                  alt=""
        ...
        <el-table-column prop="details" label="项目详情" align="center">
          <template slot-scope="scope">
            ...
            <el-button
              @click.native.prevent="jump(tableDatass[scope.$index].id)"
              type="text"
              size="small"
              style="color:#409EFF;"
            >
        ...
        <el-table-column label="操作" width="200" align="center" prop="status">
          <template slot-scope="scope">
            <span v-if="1 == 2" :v-model="tableDatass[scope.$index].id">{{
              tableDatass[scope.$index].id
            }}</span>
            <el-button
              @click.native.prevent="
                dugeline(scope.$index, tableDatass[scope.$index].id)
              "
              type="text"
              size="small"
              v-if="tableDatass[scope.$index].showStatus == 0"
            >
            </el-button>
            <el-button
              v-if="
                tableDatass[scope.$index].showStatus == 1 &&
                  tableDatass[scope.$index].publishStatus == 0
              "
              @click.native.prevent="
                dugeline(scope.$index, tableDatass[scope.$index].id)
              "
            ...
            <el-button
              v-if="
                tableDatass[scope.$index].showStatus == 1 &&
                  tableDatass[scope.$index].publishStatus == 1
              "
              @click.native.prevent="
                dugeline(scope.$index, tableDatass[scope.$index].id)
              "
      ...
      <div class="userAgree">
        ...
        <el-input
          ...
          type="textarea"
          :rows="20"
          style="width: 1200px;margin-left: 45px;margin-top: 20px;margin-bottom: 20px;"
        ></el-input>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    saveAgree() {
      ...
      this.$axios
        .post(request.testUrl + "/platform/auth2/agreementTab/addInfo", params)
        .then(res => {
          if (res.data.code == 0) {
            this.$message({
              type: "success",
    ...
    renderData(n1, n2) {
      this.loading = true;
      this.$axios
        .get(request.testUrl + "/project/auth2/project/pageFindProjectList", {
          params: {
            pageSize: n1,
            page: n2
          }
        ...
        .then(res => {
          ...
          if (res.data.code == 0) {
            ...
            for (var i = 0; i < this.tableDatass.length; i++) {
              this.tableDatass[i].createTime = this.transformDate(
                this.tableDatass[i].createTime
              );
            }
          } else {
            this.$message({
    ...
    renderUserAgree() {
      this.$axios
        .post(request.testUrl + "/platform/auth2/agreementTab/selPlaInfo")
        .then(res => {
          if (res.data.code == 0) {
    ...
    save(form5) {
      ...
      this.$axios
        .post(request.testUrl + "/project/auth2/project/insert", params)
        .then(res => {
          if (res.data.code == 0) {
            this.$message({
              type: "success",
    ...
    dugeline(i, id) {
      ...
      if (this.tableDatass[i].publishStatus == 1) {
        ...
        this.$axios
          .post(
            request.testUrl + "/project/auth2/project/updatePublishStatus",
            params
          )
          .then(res => {
            if (res.data.code == 0) {
    ...
    del() {
      ...
      this.$axios
        .post(
          request.testUrl + "/project/auth2/project/deleteProjectBatch",
          params
        )
        .then(res => {
          if (res.data.code == 0) {
    ...
    getMapData() {
      ...
      this.form5.address = JSON.parse(
        localStorage.getItem("businessAdress")
      ).address;
      this.form5.latitudes = JSON.parse(
        localStorage.getItem("businessAdress")
      ).lat;
      this.form5.longitudes = JSON.parse(
        localStorage.getItem("businessAdress")
      ).lng;
    }
  }
